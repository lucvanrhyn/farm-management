/**
 * migrate-basson-base-schema-catchup.ts — One-shot schema catchup for basson-boerdery
 *
 * Background: basson-boerdery (FarmTrack's first paying client, 103 animals) was
 * provisioned before several base-schema tables existed. A 2026-04-19 audit vs
 * trio-b-boerdery surfaced 18 tables present on trio-b but missing on basson:
 *
 ...
 * column types observed live on trio-b-boerdery — no `prisma db push` (CLAUDE.md
 ...
 *
 * Scope: hard-coded to slug='basson-boerdery'. Do NOT generalise this to all
 * tenants — trio-b-boerdery already has these tables, and other tenants must
...
const TARGET_SLUG = 'basson-boerdery';

// ── Table DDL — copied from live trio-b-boerdery sqlite_master ─────────────
...
// All non-PK indices observed on trio-b-boerdery, with IF NOT EXISTS added.
...
async function main(): Promise<void> {
  console.log(
    `\n-- basson-boerdery base-schema catchup (Notification + PushSubscription + FarmSpeciesSettings + AlertPreference + Game*) --\n`,
  );
