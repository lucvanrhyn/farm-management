...
async function main() {
  ...
  await prisma.farmSettings.upsert({
    where: { id: "singleton" },
    update: { farmName: "Delta Livestock", breed: "Brangus" },
    create: { id: "singleton", farmName: "Delta Livestock", breed: "Brangus", alertThresholdHours: 48 },
  });

  const total = await prisma.camp.count();
  console.log(`✓ Seeded camps: ${CAMPS.length} processed, ${total} total in DB`);
  console.log(`✓ FarmSettings updated: farmName="Delta Livestock", breed="Brangus"`);
