 ...
 * Background
 * ----------
 * acme-cattle (FarmTrack's first paying client, 103 animals) was
 ...
 *      `npx dotenv-cli -e .env.local -- npx tsx scripts/backfill-animal-species.ts --slug=acme-cattle`
 ...
 *      and by querying acme-cattle with "how many cattle do I have"
 *      (must now return a non-zero answer grounded in animal chunks).
 *
 * Scope: defaults to acme-cattle only. Pass `--all` to walk the full
...
function parseCliArgs(argv: ReadonlyArray<string>): CliArgs {
  ...
  // Default target = acme-cattle (the known affected tenant) when
  // nothing else was passed. Mirrors migrate-basson-base-schema-catchup.ts.
  if (!all && slug == null) slug = 'acme-cattle';
