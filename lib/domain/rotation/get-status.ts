/**
 * Wave G2 (#166) — domain op `getRotationStatusByCamp`.
 *
 * Owns the rotation-engine read-side business logic (per-camp status,
 * grazed/rested days, next-to-graze ranking, season multiplier).
 *
 * This module is the canonical home for rotation read state. The legacy
 * `lib/server/rotation-engine.ts` is reduced to a re-export shim of this
 * module so the four outside consumers (dashboard-alerts, cached.ts,
 * rotation-planner page, camp detail page) keep resolving without edits.
 *
 * Behaviour preserved verbatim from the pre-G2 `lib/server/rotation-engine.ts`
 * — only the file location moved. Consumers that import the types
 * (`CampRotationStatus`, `RotationPayload`, `RotationMobSummary`) keep
...
function isValidVeldType(v: string | null | undefined): v is VeldType {
  return (
    v === "sweetveld" || v === "sourveld" || v === "mixedveld" || v === "cultivated"
  );
}

/**
 * Builds a map of campId → { departedAt, arrivedAt } by scanning all
 * mob_movement observations. "departedAt" = most recent time a mob left
 * the camp. "arrivedAt" = most recent time a mob entered the camp.
 *
 * Observations are already indexed on (type, campId, observedAt), so this
 * query uses that index. We walk the full result set in memory — safe for
 * farms with hundreds of camps and thousands of moves. If this ever gets
 * hot, denormalise onto the Camp row.
 */
async function getLatestMovesByCamp(
  ...
  // campId = sourceCamp, one with campId = destCamp. We classify each row
  // by parsing its details JSON. Rows are ordered observedAt desc, so the
  // first hit per (camp, direction) wins.
  ...
  for (const row of rows) {
    ...
    result.set(row.campId, {
      departedAt:
        isSource && current.departedAt == null ? observedAt : current.departedAt,
 ...
 *
 * `now` is injectable to make this testable (and to pin the reporting
 * clock for a request). Defaults to `new Date()` if omitted.
 */
export async function getRotationStatusByCamp(
  ...
  const campStatuses: CampRotationStatus[] = camps.map((camp) => {
    ...
    const effectiveRestDays = resolveEffectiveRestDays(
      campConfig,
      settings,
      now,
      veldScore,
    ...
    if (isOccupied) {
      ...
      // arrival record (animals existed before rotation tracking), fall
      // back to creation date — conservative, better than null for
      // classification.
      if (lastArrivedAt) {
