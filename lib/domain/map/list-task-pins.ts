/**
 * Wave G3 (#167) — domain op `listTaskPins`.
 *
 * Returns a GeoJSON `FeatureCollection` of Point features, one per Task
 * with a resolvable coordinate.
 *
 ...
 * `app/api/[farmSlug]/map/task-pins/route.ts`.
 */
...
function geometryCentroid(geom: unknown): [number, number] | null {
...
export async function listTaskPins(
  prisma: PrismaClient,
  status: TaskPinStatusFilter,
): Promise<GeoJsonFeatureCollection<TaskPinProperties>> {
