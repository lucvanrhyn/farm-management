...
/**
 * POST /api/animals/import — bulk XLSX importer.
 *
 * Returns a Server-Sent-Events stream of progress events. Wave A wraps it
 * in `adminWrite` so auth, role gate, fresh-admin re-verify, server-timing,
 * and the typed-error envelope on throw are inherited from the adapter.
 *
 * The body is multipart/form-data so the adapter skips JSON parse and the
 * handler reads `req.formData()` directly. The response is a streaming
 * `Response` (not `NextResponse`); revalidation fires once the stream has
 * finished — but since the adapter sees the streaming Response as a 2xx,
 * it kicks off `revalidateAnimalWrite(slug)` immediately. That mirrors
 * the legacy behaviour where revalidate happened inside the stream's
 * `start` callback after the last batch.
 */
export const POST = adminWrite({
  ...
  handle: async (ctx, _body, req) => {
    ...
    if (workbook.sheetNames.includes("Camps")) {
      const campRows = readSheetAsObjects(workbook, "Camps", {
        defval: "",
          ...
          try {
            await prisma.camp.create({
              data: {
                ...
                campName,
                sizeHectares:
                  sizeHectares !== null && !isNaN(sizeHectares)
                    ? sizeHectares
                    : null,
                waterSource,
              },
    ...
    // Two-tab template: use "Animals" sheet. Single-sheet (legacy): use
    // first sheet.
    const animalSheetName = workbook.sheetNames.includes("Animals")
      ? "Animals"
      : workbook.sheetNames[0];
    const rows = readSheetAsObjects(workbook, animalSheetName, {
      defval: "",
    }) as Record<string, string>[];
      ...
      async start(controller) {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          if (!animalId) {
            ...
            if (!sex) {
              errors.push(
                `Row ${rowNum} (${animalId}): invalid sex "${row.sex}" — expected Male/Female or Manlik/Vroulik`,
              );
              skipped++;
            } else {
              ...
              if (!category) {
                errors.push(
                  `Row ${rowNum} (${animalId}): invalid category "${row.category}" — expected Cow/Koei, Bull/Bul, Heifer/Vers, Calf/Kalf, or Ox/Os`,
                );
                skipped++;
              } else {
                ...
                if (!currentCamp) {
                  errors.push(
                    `Row ${rowNum} (${animalId}): missing current_camp`,
                  );
                  skipped++;
                } else if (
                  hasCampValidation &&
                  !validCampIds.has(currentCamp)
                ) {
                  errors.push(
                    `Row ${rowNum} (${animalId}): camp "${currentCamp}" not found — create it in the Camps sheet or add it manually first`,
                  );
                  skipped++;
                } else {
                  const status = String(row.status ?? "Active").trim();
                  const resolvedStatus = VALID_STATUSES.has(status)
                    ? status
                    : "Active";
