 ...
 * bare-string `{ error }` 4xx envelopes, `{ success: true, pending: true }`
 * happy path, 429 rate-limit envelope, 500 generic-error envelope. The
 * anti-enumeration timing match (dummy `hash(password, 12)` on existing-email
 * branch) stays inside `handle` — adapter does not alter wall-clock cost.
 */
