...
- LHCI workflow: runs on every PR to main, measures 5 URLs (/login, /trio-b-boerdery/home, /trio-b-boerdery/dashboard, /trio-b-boerdery/logger, /trio-b-boerdery/admin/animals) with `--clear-storage` via a bench user `e2e-bench@farmtrack.app` (add a secret BENCH_PASSWORD). Budgets: FCP<2s, LCP<3s, TTI<4s, login total JS<300KB brotli.
...
- Manually visit /trio-b-boerdery/logger in dev with DevTools Network tab → exactly one /api/camps entry, exactly one /api/animals entry on cold load.
...
2. __tests__/admin/animals-page-html-size.test.ts — hit /trio-b-boerdery/admin/animals with next-test; assert HTML < 100 KB.
...
- `pnpm tsc --noEmit` clean.
- Manual: visit /trio-b-boerdery/admin/animals in dev; view-source; confirm HTML < 100 KB; confirm scroll loads the next 50.
...
**Success gate:**
- Cold TTFB on `/trio-b-boerdery/dashboard` drops to < 1 s (from Wave 1 baseline ~2 s — Wave 1 should have halved from the original 3–4 s).
...
After all 7 phases, running `./scripts/bench-cold.sh trio-b-boerdery` from Cape Town produces:

```
/login                          FCP  < 1.0 s   JS   <  90 KB brotli
/trio-b-boerdery/home           TTFB < 0.6 s
/trio-b-boerdery/dashboard      TTFB < 0.8 s   LCP  <  1.8 s
/trio-b-boerdery/logger         TTFB < 0.6 s   cold API fan-out ≤ 4 calls
/trio-b-boerdery/admin/animals  HTML < 100 KB  TTI  <  2.5 s
```
