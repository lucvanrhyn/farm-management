...
**Scope:** Production https://farm-management-lilac.vercel.app, measured via playwright-cli from Cape Town. Tenant: `delta-livestock` (874 animals, 19 camps).
...
| `/delta-livestock/home`               | 1.50 s    | 1.62 s   | 18 KB    | Fires `/api/farm` + vitals after paint.       |
| `/delta-livestock/dashboard` (SSR)    | **4.14 s** | 4.52 s  | 59 KB    | Then `/api/camps/status` adds 2.11 s.         |
| `/delta-livestock/admin` (SSR)        | 2.24 s    | 5.17 s   | 61 KB    | `/api/notifications` alone 3.57 s.            |
| `/delta-livestock/admin/animals` SSR  | 2.90 s    | 4.79 s   | **557 KB** | Renders all 874 animals into HTML, no LIMIT. |
| `/delta-livestock/logger`             | 1.57 s    | 2.07 s   | 19 KB    | Fan-out of 7 API calls, see below.            |
...
| `/delta-livestock/dashboard` SSR   | 3.60 s      | 3.02 s | 3.11 s |       |       |
...
- `.env.local` shows `TURSO_DATABASE_URL=libsql://delta-livestock-lucvanrhyn.aws-ap-northeast-1.turso.io` → the Turso DB lives in **Tokyo**.
