...
- **(b) Skip clone, run preview against source DB read-only** — rejected. The source DB (`basson-boerdery-dub`) is the prod tenant; preview writes would mutate prod. No "read-only mode" flag exists, and adding one is bigger than the install fix.
