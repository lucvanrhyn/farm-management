...
5. **Reads tolerate NULL.** ~~Until the backfill migration lands and any orphan-NULL rows are audited, `scoped(mode).observation.findMany(...)` resolves to `where: { OR: [{ species: mode }, { species: null }] }`. The NULL-tolerant branch is removed in a follow-up cleanup wave (out of scope for #231) once the audit confirms zero orphan rows in prod.~~ **Closed by ADR-0006 (2026-05-21).** The OR-branch was never actually committed to `scoped().observation` — every op shipped at #231 already filtered cleanly on `{ species: mode }`. The contract sat exposed because three write-seam bypassers (`app/api/animals/[id]/photos/route.ts`, `lib/domain/tasks/update-task.ts`, `lib/domain/mobs/move-mob.ts`) kept producing fresh NULL-species rows that the predicate silently dropped from every per-species read. ADR-0006 closes the write seam (one named door, structurally enforced), ships migration `0024_backfill_observation_species_stragglers.sql` to drain the accumulated NULL tail, and locks the predicate at `{ species: mode }` with a load-bearing comment in `lib/server/species-scoped-prisma.ts` so the OR-branch can never be reintroduced. See `docs/adr/0006-observation-write-named-door.md`.
