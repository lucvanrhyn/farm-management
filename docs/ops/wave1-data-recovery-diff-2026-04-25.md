...
## Tenant 1 — `trio-b-boerdery`

- **Source (Tokyo)**  : `libsql://trio-b-boerdery-lucvanrhyn.aws-ap-northeast-1.turso.io`
- **Target (Ireland)**: `libsql://trio-b-boerdery-dub-lucvanrhyn.aws-eu-west-1.turso.io`
- **Tables compared** : 43
...
## Tenant 2 — `basson-boerdery`

- **Source (Tokyo)**  : `libsql://ft-basson-boerdery-lucvanrhyn.aws-ap-northeast-1.turso.io`
- **Target (Ireland)**: `libsql://basson-boerdery-dub-lucvanrhyn.aws-eu-west-1.turso.io`
- **Tables compared** : 43
- **Tables diverged** : 1 (`Notification`)
- **Rows lost (source > target)** : **0**
- **Rows ahead (target > source)** : 4 (post-cutover cron writes — expected)

> Naming note: the meta DB tenant slug is `basson-boerdery`; the legacy Tokyo
> DB host was `ft-basson-boerdery-lucvanrhyn.aws-ap-northeast-1.turso.io`
> (the `ft-` prefix was an early provisioning quirk that never made it into
> the slug). MEMORY.md uses `ft-basson-boerdery` to refer to the source DB.
...
Same pattern as Trio-B — daily notification cron post-cutover. No replay.
...
### `trio-b-boerdery` Notification (target only — post-cutover)
...
### `basson-boerdery` Notification (target only — post-cutover)
...
not in active interactive use during cutover windows — Trio-B and Basson are
...
1. **2026-05-09 (or earlier if soak window shortens)**: re-run this diff
   ...
   stays "no lost writes", run `turso db destroy ft-basson-boerdery` and
   `turso db destroy trio-b-boerdery`, then clear `legacy_turso_url` and
