...
describe('resolveCriticalRoutes', () => {
  it('substitutes the farm slug into every route', () => {
    ...
    for (const r of resolved) expect(r.url).toMatch(/^\/delta-livestock/);
  });
  ...
  it('substitutes [campId] when provided', () => {
    ...
    expect(resolved.some((r) => r.url === '/delta-livestock/admin/camps/A')).toBe(true);
  });

  it('throws when [campId] route is in scope but no campId was provided', () => {
    expect(() => resolveCriticalRoutes({ farmSlug: 'delta-livestock' })).toThrow(
      /needs a campId/,
    );
  });
