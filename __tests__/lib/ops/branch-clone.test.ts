...
// added beyond Acme Cattle would silently miss schema changes at promote
...
describe('runProdMigrationsAllTenants — multi-tenant enumeration (Wave 4 A1)', () => {
  it('runs migrations against ≥2 tenants when meta-DB has multiple farms', async () => {
    await insertFarmRow(memClient, { slug: 'acme-cattle' });
    ...
    expect(calledSlugs).toEqual(['acme-cattle', 'tenant-three', 'tenant-two']);

    // Each tenant's applied list contributes to the aggregate, prefixed by slug.
    expect(result.applied).toContain('acme-cattle:acme-cattle-0001.sql');
