 ...
 * 3514534429) on prod for trio-b-boerdery, making the entire logger
...
describe("LoggerPage SSR resilience (hotfix P0.2)", () => {
  ...
  it("renders the page successfully when farmSettings resolves with a name", async () => {
    findFirst.mockResolvedValueOnce({ farmName: "Trio B Boerdery" });

    const Page = (await import("@/app/[farmSlug]/logger/page")).default;
    const tree = await Page({ params: Promise.resolve({ farmSlug: "trio-b-boerdery" }) });
    ...
    expect(serialized).toContain("Trio B Boerdery");
  });
  ...
  it("does NOT throw when prisma.farmSettings.findFirst rejects (the production crash)", async () => {
    ...
    await expect(
      Page({ params: Promise.resolve({ farmSlug: "trio-b-boerdery" }) }),
    ).resolves.toBeTruthy();
  });
