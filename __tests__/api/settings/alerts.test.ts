...
vi.mock("@/lib/farm-prisma", () => ({
  ...
  // The route funnels through getFarmContextForSlug. Tests don't set signed
  // headers, so the cookie-scoped getFarmContext fast path never fires and
  // (post-#495) returns null — the helper then re-auths against the URL slug
  // via getPrismaForSlugWithAuth, the path these tests drive.
...
function req(body: unknown): NextRequest {
  return new NextRequest("http://localhost/api/delta-livestock/settings/alerts", {
    method: "PATCH",
...
function sessionOK(role: "ADMIN" | "LOGGER" = "LOGGER") {
  mockGetServerSession.mockResolvedValue({
    user: {
      id: "user-1",
      email: "u@test",
      farms: [{ slug: "delta-livestock", role }],
    },
  });
...
describe("PATCH /api/[farmSlug]/settings/alerts", () => {
  ...
  it("admin successfully writes farm-level quietHours + a pref row together", async () => {
    ...
    const res = await PATCH(req(body), { params: params() });
    expect(res.status).toBe(200);
    expect(mockVerifyFreshAdminRole).toHaveBeenCalledWith("user-1", "delta-livestock");
