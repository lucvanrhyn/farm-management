...
const PHASE_K_ROUTES: Array<{
  ...
  {
    label: "water-points — delta-livestock",
    path: "/api/delta-livestock/map/water-points",
    requiresAuth: true,
  },
  {
    label: "infrastructure — delta-livestock",
    path: "/api/delta-livestock/map/infrastructure",
    requiresAuth: true,
  },
  {
    label: "rainfall-gauges — delta-livestock",
    path: "/api/delta-livestock/map/rainfall-gauges",
    requiresAuth: true,
  },
  {
    label: "task-pins — delta-livestock",
    path: "/api/delta-livestock/map/task-pins",
    requiresAuth: true,
  },
  // Second tenant variant
  {
    label: "water-points — acme-cattle",
    path: "/api/acme-cattle/map/water-points",
    requiresAuth: true,
  },
...
describe("proxy.ts isProtectedPath — Phase C bug C2 fall-through", () => {
  it.each([
    ...
    ["/delta-livestock/admin/animals", "tenant admin"],
...
describe("middleware.ts config.matcher — edge cases", () => {
  ...
  it("guards all tenant slugs — not just delta-livestock", () => {
    const slugs = ["delta-livestock", "acme-cattle", "my-farm-2026", "test-tenant"];
