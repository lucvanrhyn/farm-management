 ...
 *
 * __tests__/app/game-layout-auth.test.tsx
 *
 * Deep-link consistency (#544): app/[farmSlug]/game/layout.tsx must bounce
 * unauthenticated visitors to /login?next=/<slug>/game rather than dropping
...
vi.mock('@/components/game/GameSubNav', () => ({ default: () => null }));
...
async function runLayout(
  ...
  const { default: GameLayout } = await import('@/app/[farmSlug]/game/layout');
  try {
    const result = await GameLayout({
      children: null,
...
describe('GameLayout — auth gate (#544)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  ...
  it('redirects unauthenticated users to /login?next=/<slug>/game (deep-link)', async () => {
    ...
    expect(redirected).toBe('/login?next=%2Facme-farm%2Fgame');
  });
